...
/**
 * Build a binary buffer for an Android touch event compatible with scrcpy's
 * control protocol.
 *
 * @param action      AMOTION_EVENT.* code
 * @param pointerId   Pointer identifier (-1 for mouse)
 * @param x           X coordinate in video space (pixels)
 * @param y           Y coordinate in video space (pixels)
 * @param videoWidth  Width of the video stream (pixels)
 * @param videoHeight Height of the video stream (pixels)
 * @param pressure    0–1 float (defaults to 1.0)
 * @param actionButton Which button triggered the action (usually 0)
 * @param buttons     Bit-field of pressed buttons.
 */
